...
class App extends Component {
  ...
  handleReceiveSongs = () => {
    ...
    fetch(`https://peaceful-meadow-03064.herokuapp.com/songs?${decadeQuery}&numSongs=${numSongs}`)
      .then(res => res.json())
      .then((songsArray) => {
  ...
  handleSendPoints = () => {
    ...
    this.state.players.map(player => {
      fetch(`https://peaceful-meadow-03064.herokuapp.com/users/${player.id}`, {
        method: "PATCH",
        headers: {
  ...
  createUserGames = () => {
    this.state.players.map(player => {
      fetch("https://peaceful-meadow-03064.herokuapp.com/user_games", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  handleLogin = (userObj) => {
    ...
    if (userExists === undefined) {
      let searchedObj = {}
      fetch(`https://peaceful-meadow-03064.herokuapp.com/users?username=${userObj.username}&password=${userObj.password}`)
        .then(res => res.json())
  ...
  handleRegister = (userObj) => {
    ...
    if (userObj.username.length > 0) {
      let searchedObj = {}
      fetch(`https://peaceful-meadow-03064.herokuapp.com/check_user?username=${userObj.username}`)
        .then(res => res.json())
        .then(searchedUserObj => {
          searchedObj = searchedUserObj
          if (searchedObj.message === "User does not exist") {
            fetch("https://peaceful-meadow-03064.herokuapp.com/users", {
              method: "POST",
              headers: {
                "Content-Type": "application/json"
              },
  ...
  handleNewGame = () => {
    ...
    fetch("https://peaceful-meadow-03064.herokuapp.com/games", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
